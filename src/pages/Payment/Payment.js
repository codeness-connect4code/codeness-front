...
const Payment = () => {
   ...
   useEffect(() => {
       ...
       setToken(jwtToken);
       axios.defaults.headers.common['Authorization'] = jwtToken.startsWith('Bearer ') 
           ? jwtToken 
           : `Bearer ${jwtToken}`;
   ...
   const handleApply = async (scheduleId) => {
       ...
       try {
           ...
           console.error('주문 생성 요청 중 오류:', error.response?.data || error.message);
   ...
   const handlePayment = () => {
       ...
       window.IMP.request_pay(paymentData, async (rsp) => {
           ...
           if (rsp.success) {
               ...
               setPgTid(rsp.pg_tid);
               try {
                   const response = await axios.post(`/mentoring/payments/${paymentId}/verify`, {
                   ...
                   console.error('결제 검증 요청 중 오류:', error.response?.data || error.message);
   ...
   const handleRefund = async () => {
       ...
       setIsLoading(true);
       try {
           const response = await axios.post(`/payments/${paymentId}/refund`, { pgTid });
           alert(`환불 완료: ${response.data.msg}`);
       } catch (error) {
           console.error('환불 요청 중 오류:', error.response?.data || error.message);
        ...
        {schedules.length > 0 ? (
            <ul>
                {schedules.map((schedule) => (
                    <li key={schedule.id}>
                        <span>스케쥴: {schedule.date} {schedule.time}</span>
                        <div>
                            {/* 신청 버튼 */}
                            <button
